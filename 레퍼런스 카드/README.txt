레퍼런스 카드 쇼츠 — 작업 폴더

소재는 이 폴더가 아니라 아래 데이터셋에서 읽습니다. 여기에는 기록만 쌓입니다.

  소재   research\single-screen-references\videos.jsonl   (2,000건)
  회로   하루건강약사 - 레퍼런스 카드 쇼츠

한 번 실행하면 아직 안 쓴 레코드 1건을 무작위로 골라, 사용자가 재가공해 둔
`*_reworked_ko` 문안 그대로 카드 이미지를 만들고 BGM → 5초 MP4 → YouTube 공개
업로드 → 고정 댓글 → 사용기록 체크까지 처리합니다.

이미지 생성·BGM·렌더·업로드 노드는 메인 회로(하루건강약사 - n8n 유튜브 쇼츠 자동화)에서
그대로 복제한 것이라 그림 품질과 음악 규칙이 기존 쇼츠와 같습니다. 카드 마무리 줄에는
카드뉴스처럼 `@haruyaksa`가 붙습니다.


기록 (= 체크리스트)

  기록\사용기록.jsonl    이미 쓴 record_id. 여기 있는 레코드는 다시 뽑히지 않습니다.
  기록\업로드기록.jsonl  업로드까지 성공한 건만.
  기록\reference-card.lock  실행 중 잠금. 30분 지나면 자동으로 풀립니다.

렌더까지 갔으면 업로드 실패 여부와 무관하게 사용기록에 남습니다. 비용이 이미 났고, 같은
레코드를 또 뽑아 쓰는 것이 더 나쁩니다. 다시 쓰려면 사용기록에서 그 줄을 지우세요.

의학 안전 검수에서 막힌 건은 사용기록에 남지 않습니다. 문안을 고치거나 다른 레코드로
다시 실행하면 됩니다.


재가공 문안 쓰는 법

회로는 시트의 *_reworked_ko 문안을 한 글자도 안 고치고 그대로 카드에 찍습니다.
AI 검수도 안 탑니다. 그러니 문안 품질은 시트에 쓸 때 결정됩니다.
n8n 회로 전체에 적용한 규칙과 같습니다(2026-08-04).

  줄이려다 뜻이 사라지면 실패입니다.
    글자를 아끼려고 주어·목적어·조사·상황을 빼지 마세요. 항목 이름 40자,
    설명 90자까지 쓸 수 있고 이건 상한이지 목표가 아닙니다. 읽는 사람이
    해독해야 하는 문장은 30자여도 실패고, 술술 읽히면 85자여도 괜찮습니다.
      나쁨  넓은 손잡이            (무엇의 손잡이인지 카드만 봐선 모름)
      좋음  손잡이 넓은 냄비

  말로 하는 말을 쓰세요. 정확한 단어라도 아무도 입으로 안 하면 틀린 단어입니다.
      손아귀 → 손힘        상온 → 그냥 두면, 밖에 두면
      단면 → 자른 자리      눈금 → 숫자         기한 → 날짜
      섭취·유의·권장·실시 → 먹다·보다·권하다
    60대 두 사람이 식탁에서 나누는 말로 읽어 보세요. 상자에 붙은 라벨처럼
    들리면 다시 쓰세요.

  수동형과 ~게 되다를 쓰지 마세요. 누가 무엇을 하는지 밝힙니다.
      목이 꺾인 자세로 자게 돼요  →  고개가 꺾인 채로 자요
      상자째 베란다로 밀려나요     →  상자째 베란다로 내놔요
    한국어가 실제로 쓰는 피동(문이 닫혀요)은 괜찮습니다.

  비유와 돌려 말하기를 빼세요. 무엇을, 어떻게 하면, 어떻게 되는지 한 줄에 담습니다.

  제목은 후킹으로 씁니다. 분류 라벨이면 실패입니다. 다만 항목이 실제로 지키는
  약속만 거세요 — 없는 숫자, 없는 권위, 질병 공포, 보장된 결과는 금지입니다.

  부제(card_headline_reworked_ko)는 카드에 안 그립니다. 유튜브 설명에만 들어갑니다.

  항목 수는 render_max_items(기본 7)까지만 카드에 올라갑니다. 그보다 많이 쓰면
  앞에서부터 잘리고 제목의 개수도 함께 내려갑니다.


선별 기준 — selection-gate.json

데이터셋 자체 QA가 2,000건 중 `publish_ready`를 11건만 true로 두고 있습니다.
(`claim_risk` high 1,945건, `fact_check_required` true 1,975건.) high로 잡힌 것은
대부분 낙상·운동·시니어 건강처럼 의학적 주장이 섞인 주제이고, publish_ready 11건은
요리·관계·생활습관 주제입니다.

기본 기준은 그 플래그를 그대로 존중합니다. 넓히려면 이 파일을 고치세요. 회로를 다시
빌드할 필요 없이 다음 실행부터 적용됩니다.

  require_publish_ready      true면 publish_ready=true만 씁니다.
  allowed_claim_risk         ["low"] → ["low","medium"] 처럼 넓힐 수 있습니다.
  allow_fact_check_required  true로 두면 사실확인 필요 건도 씁니다.
  min_items / max_items      카드 항목 수 범위.

의학성 주제를 열 때는 약사 검수를 먼저 거치는 것을 전제로 합니다. 회로의
`Medical Safety Review`는 완치·치료 보장·진료 회피 같은 표현만 막고, 개별 사실의
정확성은 검사하지 않습니다.


Google Sheet 체크박스

원본 시트(`한 화면 정보 카드 레퍼런스 1-2000`, ID 1K6gT9TY_WHuxB3SHEx5VyJK2JunQWJRdkdV4ecNu_fc)의
`통과 영상` 탭 `AU` 열(`업로드 완료`)을 이 회로가 동기화합니다.

실행 시작 시:
  - 시트의 2,000행을 `record_id`로 videos.jsonl에 병합합니다.
  - 기존 JSONL 레코드 개수와 순서는 바꾸지 않습니다.
  - `사용기록.jsonl`에 있는 record_id의 `AU` 체크박스를 Boolean TRUE로 맞춥니다.

렌더 이후:
  - `사용기록.jsonl`을 기록한 뒤 현재 record_id의 `AU` 체크박스를 즉시 TRUE로 바꿉니다.
  - 업로드가 실패해도 렌더 비용이 발생했으면 체크합니다.

필요한 n8n 자격 증명은 `Google Sheets account`입니다. OAuth 연결이 끊기면 소재 선택 전에
동기화 단계에서 실패하므로 오래된 로컬 문안으로 게시되지 않습니다.
자격 증명을 다시 만들 때는 n8n을 먼저 끈 뒤 `npm run setup:reference-card-sheets-auth`를
실행하고, n8n 자격 증명 화면에서 Google 계정을 연결하세요.
로컬 n8n 로그인을 사용할 수 없으면 n8n을 끈 뒤
`npm run authorize:reference-card-sheets`를 실행해 Google 승인 화면에서 연결할 수 있습니다.
이미지 생성이나 공개 업로드 없이 동기화 네 노드만 점검하려면 n8n을 끈 뒤
`npm run smoke:reference-card-sheets`를 실행하세요.


주의: 실행하면 KIE 비용이 발생하고 YouTube에 공개 영상이 게시됩니다. 회로를 가져오거나
열기만 해서는 실행되지 않습니다.
