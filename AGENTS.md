...
- Instagram publish contract: the rendered source must remain 1080x1920 (9:16), but that alone does not protect the external upload. The Instagram web crop screen must explicitly select `9:16`; selecting `원본` is not evidence. Block `공유하기` when the crop preview is square, and after publishing inspect the Reel page's actual `video.videoWidth/video.videoHeight` before any P7/P8 post-work. A square Reel is a failed publish, not a thumbnail variant.
