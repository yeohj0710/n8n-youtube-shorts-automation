...
- **Row copy lives in a band: `card_name` ≤ 12 characters, `card_reason` 18–28, printed in at most two lines (2026-08-06 evening).** Both edges of the band are published failures. 50–90 character sentences wrapped three times and crushed the type to 30 px. Then the 16-character cap over-corrected: the sentences lost their subject and context and the user could not tell what they meant (`자국이 남으면 물이 찼어요` — press what, where?). 18–28 characters holds one complete spoken sentence with its subject and consequence — `누른 자국이 한참 남으면 물이 찬 거예요` — and the measured type stays at 46 px+ across two lines. Rows stay at 4 (writer default 4, max 5).
