...
- Instagram publish contract: the rendered source must remain 1080x1920 (9:16) and use Instagram-safe H.264 video plus AAC audio in an MP4 with faststart; AV1+Opus can produce a black profile-grid cover. The Instagram web crop screen must explicitly select `9:16`; selecting `원본` is not evidence. In the cover editor, explicitly choose a normal card frame instead of a black opening frame. Block `공유하기` when the crop preview is square, and after publishing inspect both the Reel page's actual `video.videoWidth/video.videoHeight` and the profile Reels grid tile before any P7/P8 post-work. A square Reel or black grid cover is a failed publish, not a thumbnail variant.
...
- **Same story, different title — the guard cannot see it, so a verifier now does.** `TOPIC_DUPLICATE_GUARD_V1` compares titles only. Five fall-prevention packs shipped in a row (욕실 미끄러짐, 장롱 위 물건, 밤 화장실, 다리 힘 신호, 집 조명) because their titles share almost no words; two of them even carried the same row (밤 화장실 길에 발밑 등). Three of the five were staged in one writing batch, which is where the range collapsed. `verify-queued-topics-are-fresh.mjs` now compares **row content** — queued packs against each other and against the published archive in `<채널> 소재/사용완료` — and fails `npm test` at 34% content-word overlap. When it fires, pick a different subject; rewording the title is exactly the failure it exists to catch. Two traps: an intentional republish must move its old archive copy to `보류` first, or it matches itself; and generic title tails like `알아 둘 것 4` collide with the token-overlap guard across channels, so make titles carry topic words.
...
- **The owner widened the topic space on purpose (2026-08-08).** Viral life-utility topics are wanted, not tolerated: 환급금·잠자는 돈, 노후 일자리, 분실물 찾는 절차, 층간소음 같은 생활 민원, 위급 대비 휴대폰 설정, 응급 신호, 뜻밖에 위험한 생활 습관. The boundary the owner set: never 100% opposite to the channel's identity — keep some intersection with a 50+ viewer's health and daily function, but the overlap may be loose. Unused seeds for later batches: 전세사기·보증금 예방, 버스 분실물, 무조건 쉬어야 하는 몸 상태. Four rules learned staging the first batch of these:
  - Emergency-symptom topics (bar 119, ER signs) are queue-only and hand-written from textbook signs — never left to on-the-fly AI generation, because a hallucinated symptom row is a real-world harm, and the owner flagged exactly this ("위험해서 정확히 적어야 함").
  - Institution facts — service names, hotlines (내보험찾아줌, 로스트112, 1661-2642) — go in only when they really exist and are stable; record the verification in the pack's `notes`. Never let the writer AI invent an agency or a number.
  - Fear frames get reframed into concrete strain: the owner's own example 자다가 돌연사하는 습관 shipped as 심장이 밤새 고생하는 자기 전 습관 — same pull, no death scare, every row still evidence-solid.
  - Money wording must be literally true: 사라지는 내 돈 is only allowed because 소멸시효 actually extinguishes those claims. If the money does not expire, do not say it disappears.
