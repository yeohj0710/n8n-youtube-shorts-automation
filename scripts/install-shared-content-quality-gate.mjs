...
function inspectPack(value, researchPack) {
  ...
  for (const [index, item] of items.entries()) {
    ...
    if ([...cardReason].length > 16) issues.push({ rank, code: 'card_copy_too_long', message: 'Mobile card sentence exceeds 16 characters.', length: [...cardReason].length });
...
const prompt = [
  ...
  'I5. COMMON_KNOWLEDGE_V1: apply the 나도 알지 test to every rank. If a typical Korean viewer in their late fifties would answer 그건 나도 알지 — 의자에 올라서면 위험하다, 어두우면 잘 안 보인다, 물을 충분히 마셔라 — the row teaches nothing and must be rejected with issue code low_information_value. A published card failed exactly this way: five common-sense caution rows, zero takeaways. Every row must hand the viewer something they could repeat to a friend: a surprising cause, a nameable trick, or a specific place, time, function, or interval. 얼린 두부는 식감이 고기처럼 변한다 passes; 넘어지면 다친다 does not. Judge the pack as a whole as well: when most rows are common knowledge, reject the topic itself rather than asking for rewording.',
  ...
  'L4. KOREAN_VOICE_V1: The copy must read as Korean somebody spoke, not as English carried across into Korean words. Four habits give it away, and all four are grounds to reject with issue code translationese_copy. (a) Subjects English requires but Korean drops: a Korean speaker omits the subject the situation already makes obvious, so 두 식구가 큰 통을 다 쓰기 전에 냄새가 변해요 should be 큰 통은 다 쓰기도 전에 냄새부터 변해요. (b) Inanimate things driving transitive verbs: 소음이 말소리를 덮어요 is English word order; Korean says 소음 때문에 말이 안 들려요. (c) One sentence shape repeated down the whole list, especially the if-then 조건절 — vary it with 대조 (~는데, ~지만), plain statement, cause (~어서), and an occasional courteous ~해 보세요 so the ranks do not scan identically. (d) Passive and causative piled up where Korean prefers an active verb. (e) Written-only vocabulary where a spoken word exists: 손아귀 instead of 손힘, 상온 instead of 그냥 두면, 단면 instead of 자른 자리, 눈금 instead of 숫자, 기한 instead of 날짜, 섭취/유의/권장 instead of 먹다/보다/권하다. The precise word is the wrong word when nobody says it aloud — reject with translationese_copy. (f) Any passive or ~게 되다 the speaker would not choose: 자게 돼요 for 자요, 밀려나요 for 내놔요, 눌려서 for 무게를 받으면. Idiomatic passives Korean genuinely uses (문이 닫혀요) pass; hedged ones do not. DIGNIFIED_VOICE_V1: the channel voice is a courteous professional in their fifties addressing viewers of the same generation as respected peers — composed, warm, polite 해요체 with honorific 시 for viewer actions. Reject chatty endings such as 거든요, 잖아요, or clipped ~죠 with issue code channel_tone_mismatch. Read every line aloud in your head and ask whether that professional would say it to a customer they respect. Judge hook_title and subtitle by the same test, and watch for the commonest title failure: the bookish metaphor built from an abstract noun and a figurative transitive verb, as in 결과를 흔드는 실수, 건강을 깎는 습관, or 노후를 지키는 선택 — printed-copy phrasings nobody says aloud. Reject such a title with translationese_copy and require the actual effect stated plainly, e.g. 혈압이 다르게 나오는 이유.',
  'L5. DIRECTION_MATCH_V1: read each card_name and its card_reason as one breath and check they point the same way. Reject with issue code low_information_value any row where the name states a mistake but the reason describes the benefit of the opposite action, forcing the viewer to invert the sentence in their head; any row where a plain fact is dressed as an action by nominalizing it with ~하기; any card_name that nominalizes a full clause with its subject still attached, since 자국이 오래 남기 and 주변이 시끄럽기 are broken Korean — a signal takes a 관형형 noun phrase and a check-list item takes ~는지; and any reason that leads with corrective advice instead of the direct consequence or meaning of the named thing.',
...
// Prepared packs used to skip the AI reviewer, and a queued card of pure
// common sense shipped verbatim because nothing downstream could judge
// usefulness. Prepared packs are now reviewed like generated ones; on
// rejection the retry node refuses to regenerate and stops the run instead.
...
function inspectPack(value, researchPack) {
  ...
  for (const [index, item] of items.entries()) {
    ...
    if ([...cardReason].length > 16) issues.push({ rank, code: 'card_copy_too_long', length: [...cardReason].length });
...
// A prepared pack is curated copy: regenerating it would silently replace the
// author's text, so a review failure stops the run loudly instead. The topic
// stays in the queue unconsumed; fix the file or remove it before rerunning.
if (qualityFailed && data.prepared_card_pack) {
  const failedIssues = (data.content_quality_review?.issues || []).slice(0, 8);
  throw new Error('PREPARED_PACK_REJECTED: the queued topic "' + String(data.pack?.hook_title || '') + '" failed content review and will not be regenerated. Fix or remove its queue file, then rerun. Issues: ' + JSON.stringify(failedIssues));
}
...
const qualityInstruction = [
  ...
  'Write all viewer-facing explanatory copy in natural Korean 해요체, never 합니다체. card_name is a common everyday Korean label of at most 12 characters. card_reason is ONE complete 해요체 sentence of at most 16 characters, because it has to sit on a single line at a size an older reader can actually read. Fifty-character sentences are what crushed the type down to a third of its proper size on published cards. Sixteen characters is enough for one plainly spoken sentence — 솔이 눕기 시작하면 바꿀 때예요 — and it is not enough for two facts, so choose the single most useful one, usually the cue that tells the reader it is time to act, and say it in full. Never buy those characters by dropping particles, jamming words together, or cutting the ending: a fragment the reader has to decode has failed even at ten characters. If the point cannot be said plainly in 16 characters, choose a different point. Avoid obscure object words and unexplained jargon.',
...
const medicalInstruction = [
  ...
  'Write all viewer-facing explanatory copy in natural Korean 해요체, never 합니다체. card_name is a common everyday Korean label of at most 12 characters. card_reason is ONE complete 해요체 sentence of at most 16 characters, because it has to sit on a single line at a size an older reader can actually read. Fifty-character sentences are what crushed the type down to a third of its proper size on published cards. Sixteen characters is enough for one plainly spoken sentence — 솔이 눕기 시작하면 바꿀 때예요 — and it is not enough for two facts, so choose the single most useful one, usually the cue that tells the reader it is time to act, and say it in full. Never buy those characters by dropping particles, jamming words together, or cutting the ending: a fragment the reader has to decode has failed even at ten characters. If the point cannot be said plainly in 16 characters, choose a different point.',
