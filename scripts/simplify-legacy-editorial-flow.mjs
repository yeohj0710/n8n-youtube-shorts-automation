...
function attentionPromiseContract(target) {
  ...
  return 'ATTENTION_PROMISE_V2: Before finalizing the topic, hook_title, and subtitle, create immediate truthful self-relevance for a Korean viewer over 50. The first screen must name a concrete condition, situation, action, choice, or observable signal and promise one specific decision-useful payoff. The viewer should immediately think "this may apply to me" and understand what the ranked list will resolve. subtitle is NOT drawn on the card any more — it only feeds the YouTube description — so spend the effort on hook_title and on the rows. Keep subtitle to one line that adds the missing condition or payoff rather than paraphrasing hook_title. ' + channelAngle + ' HOOK_PATTERNS: this channel already performs with a small set of proven Korean title shapes, and you should choose whichever one the researched facts actually earn: a belief reversal such as 대부분이 잘못 알고 있는 or 의외로; a loss frame such as 모르면 손해 보는, 버리면 손해 보는, or 괜히 사기 전 먼저; an insider reveal such as 의사들이 자주 말하는 or 주부 9단도 몰랐던; a head-to-head comparison such as A vs B; a targeted audience opener such as 50대 이후 or 나이 들수록; or a moment trigger such as 자기 전에, 아침에 눈뜨자마자, or 걷기 전후. Write the title the way a person actually speaks, not the way a report is labelled. A title such as 마그네슘 흡수율 비교 3가지 is a filing label and fails; 영양제 코너에서 대부분이 잘못 고르는 마그네슘 5 is the same fact written as a hook. These shapes are required to be earned, never decorative: use a loss frame only when the items really prevent a loss, an insider reveal only when the facts really are commonly missed, and a reversal only when the common belief really is wrong. Reject the shape and rewrite the title whenever the ranked items do not deliver that exact promise. INTENSITY: the channel owner has raised the aggression ceiling — go hard, not polite. Direct address to the affected viewer (무릎 아픈 분들은), real-consequence stakes (후회하기 전에, 평생 손해), urgency anchored to a real moment or cost (보청기 알아보기 전에, 병원비 쓰기 전에), strong imperatives (그냥 넘기지 마세요, 오늘부터 멈추세요), and pulling the single most surprising item detail into the title are all encouraged whenever the items genuinely back them. Prefer the stronger phrasing of two honest options. The floor that remains: no invented statistics or percentages, no invented authority or fake insiders, no disease scares or guaranteed outcomes, and no promise the ranked items do not cash.';
}
...
const clearKoreanCopyContract = 'CLEAR_KOREAN_COPY_V2: Write every viewer-facing field as idiomatic Korean that a Korean health educator would naturally say to one adult. Draft the complete reason first, then derive card_name and card_reason from that reason instead of compressing keywords independently. For every rank, read the title, card_name, and card_reason without the long reason. They must state what happens, under which condition, and the specific meaning or action without rereading or without mentally supplying a missing subject, object, particle, or situation. card_name must be a concrete standalone symptom, action, or condition in common words. card_reason must be one complete 해요체 sentence that names the observation and its specific health meaning or next action. Never use vague endings such as 확인해요 or 살펴봐요 without naming the object being checked. Do not reverse an observed loss of capacity into a cause; describe the comparable situation and the symptom that now appears. Natural short noun phrases and necessary medical terms are fine when immediately clear, but avoid report heading or search keyword list wording, forced noun chains, incompatible subject-predicate combinations, literal translation, unclear references, and repeated AI templates. SPOKEN_WORDS_ONLY_V1: use the word the viewer would say out loud, not the word a manual would print. Reject any term that only lives in writing even when it is the precise one — 손아귀 for grip (say 손힘), 상온 for room temperature (say 그냥 두면 or 밖에 두면), 단면 for cut surface (say 자른 자리), 눈금 for the marked numbers (say 숫자), 기한 for a use-by date (say 날짜), 세기 for a dial setting (name the setting), 섭취/유의/권장/실시 for eat/watch/recommend/do. Read the line aloud as one 60-year-old talking to another at the kitchen table: if it sounds like a label on a box or a line in a leaflet, rewrite it. ACTIVE_VOICE_ONLY_V1: name who or what does the thing. Korean passives and ~게 되다 hedge the sentence and read as translated English — 목이 꺾인 자세로 자게 돼요 becomes 고개가 꺾인 채로 자요, 상자째 베란다로 밀려나요 becomes 상자째 베란다로 내놔요, 아래쪽이 눌려서 becomes 위에 쌓인 무게를 받으면. Idiomatic passives Korean really does say (문이 닫혀요) are fine; the test is whether a speaker would choose that form, not whether it is grammatical. ROW_COPY_BAND_V1: card_name may run to 14 Korean characters and card_reason to 38, counted with spaces. card_reason is ONE complete 해요체 sentence of 24 to 38 characters, printed in at most two lines at full readable size. Both edges are failures this channel has already published. Past 38 characters the sentence wraps three times and the type collapses to 30 px. Under 24 it turns into a riddle: 눈이 담는 양은 한 방울보다 적어요 states a fact and leaves the reader asking what to do about it, while 눈이 담는 양은 한 방울보다 적어서 두 번째 방울은 그냥 흘러내려요 finishes the thought. Being explicit and slightly long beats being short and unclear — the channel owner has said so repeatedly. Inside the band name the condition, what happens, and what it means for the reader, keep every particle and the ending, and never buy characters by dropping a particle, gluing words together, or cutting the ending. TITLE_ROW_MATCH_V1: the title and every row must point the same direction, and this has broken published cards twice. If hook_title promises mistakes, losses, or things to avoid (잘못하고 있는 것, 안 되는 것, 실수, 손해 보는), then EVERY card_name is itself one of those mistakes and the card_reason says what goes wrong — never the correct method. If hook_title promises a method, an order, or a benefit (이렇게 하세요, 순서, 챙길 것, 좋은 것), then every card_name is one of those actions. Published failures: 잘못 붙이면 더 아파지는 파스 사용법 4 listed 막 삔 발목 - 찬 파스로 식혀요, which is the right way to do it, so the reader could not tell whether the row was a warning or an instruction; 따로 먹으면 손해 보는 음식 궁합 4 framed loss in the title and delivered 이 조합이 좋아요 in the rows. Mixing them inside one card is the same defect — do not put two instructions and two warnings in one list. Before returning JSON, read the title, then read every card_name in order, and confirm they are all the same kind of thing. If they are not, rewrite the title to match the rows rather than shipping the mismatch. Before returning JSON, perform one final native-Korean copy-edit pass over hook_title, subtitle, every name, card_name, reason, card_reason, caution, video_script, description, and pinned_comment, and rewrite any field that fails this standalone reading test.';
...
const prompt = [
  ...
  'Each ranked item must add a genuinely different medically useful point and at least two linked elements: a credible body mechanism or clinically relevant signal, plus who or when it matters, an observable condition, a practical action, or a meaningful boundary. Avoid generic slogans and obvious filler. Write reason as the complete practical cause-and-effect explanation with all facts needed for accuracy. Do not fabricate exact minutes, repetitions, percentages, thresholds, or measurements merely to sound scientific; keep a number only when it is well-established, necessary, and safe. Write card_name as an image label in common everyday Korean, at most 30 Korean characters, without obscure terms. Write card_reason as one independent 해요체 sentence for the image, at most 60 Korean characters, preserving the most useful mechanism, signal, or decision. PLAIN_MEANING_V1: clarity outranks brevity — never trim a line until a first-time reader cannot tell what it refers to. card_reason must stand on its own without card_name: name the actual thing instead of leaning on 그것, 이때, 이렇게, or a comparative with nothing to compare to, and spend the extra characters when the shorter wording loses the subject, the object, or what actually happens. Short but vague is a defect. DIRECTION_MATCH_V1: each ranked row must speak in one direction, read as one breath. When card_name names a mistake, card_reason states what that mistake directly causes — never the benefit of the opposite action: 난간을 잡지 않기 explained by 난간에 손을 얹으면 몸무게를 나눠 받아요 makes the viewer flip the sentence in their head, so write 난간 없이 디디면 무릎이 몸무게를 혼자 받아요 instead. When card_name names a signal or object, card_reason says what it means or does. Never disguise a plain fact as an action by nominalizing it with ~하기 (내려올 때 무릎이 더 버티기 is a fact wearing an action costume). Lead with the direct consequence; corrective advice comes after it or goes to the comment, never in place of it. CARD_NAME_FORM_V1: give card_name the grammatical form its role demands instead of forcing every row into ~하기. An action or mistake reads naturally as ~하기 (다리 꼬고 재기). A signal, symptom, or phenomenon is a noun phrase built with 관형형 (한참 안 없어지는 양말 자국) — never a full clause nominalized with its subject still attached, because 자국이 오래 남기 and 주변이 시끄럽기 are broken Korean. A check-list item may use ~는지 (한쪽 귀만 나빠진 건 아닌지). An object is simply the noun. NO_FIGURATIVE_COPY_V1: state things directly — no metaphor, simile, analogy, or roundabout phrasing that makes the viewer infer the point; name the actual object, the actual action, and the actual consequence. KOREAN_VOICE_V1: write Korean somebody would speak, not English carried across into Korean words. Drop the subject when the situation already makes it obvious, the way a Korean speaker does — 두 식구가 큰 통을 다 쓰기 전에 냄새가 변해요 should be 큰 통은 다 쓰기도 전에 냄새부터 변해요. Do not let an inanimate thing drive a transitive verb: 소음이 말소리를 덮어요 is English word order, and Korean says 소음 때문에 말이 안 들려요. Do not bend every ranked line into the same sentence shape, especially the if-then 조건절; mix in 대조 with ~는데 or ~지만, plain statements, cause with ~어서, and an occasional courteous suggestion with ~해 보세요 so the list does not scan identically line after line. Prefer an active verb over stacked passives and causatives. DIGNIFIED_VOICE_V1: the speaker is a courteous professional in their fifties — a pharmacist or specialist — addressing viewers of the same generation as respected peers. Keep the register composed and warm: polite 해요체 with the honorific 시 for viewer actions (살펴보세요, 확인해 보세요), never chatty endings such as 거든요, 잖아요, or clipped ~죠, no slang, no exclamation marks. Read each line aloud in your head and keep it only if that professional would say it to a customer they respect. TITLE_SPOKEN_KOREAN_V1: the same voice rules govern hook_title and subtitle, and the commonest title failure is the bookish metaphor — an abstract noun driven by a figurative transitive verb, as in 결과를 흔드는 실수, 건강을 깎는 습관, or 노후를 지키는 선택. Nobody says those sentences out loud. Name the actual effect instead: 결과를 흔드는 실수 becomes 혈압이 다르게 나오는 이유, 건강을 깎는 becomes 몸이 상하는. Before accepting any title, say it aloud in your head as one neighbour warning another; if it only works as printed copy, rewrite it. Never put 왜:, 이유:, 핵심:, or TIP: in card_reason. Write 5 ranked items by default. Use 6 or 7 when the researched facts genuinely support that many distinct points, and drop to 4 only when a fifth item would be filler. Never go below 4.',
...
function patchLoadConfig(code, target) {
  ...
  code = code
    ...
    .replace(/rank_count_max: Number\(incoming\.rank_count_max \|\| \d+\)/, 'rank_count_max: Number(incoming.rank_count_max || 5)');
...
function patchRankCountTarget(code, target) {
  code = code.replace(/const rankCountMax = Number\(cfg\.rank_count_max \|\| \d+\);/, 'const rankCountMax = Number(cfg.rank_count_max || 5);');
  ...
  return replacePatternRequired(
    ...
    ": '- Choose the ranking count between ' + rankCountMin + ' and ' + rankCountMax + '. Default to 4 items so every row keeps large readable type — the card fits four rows of one name line plus a two-line reason. Use 5 only when a fifth point is genuinely strong on its own. Do not add filler items just to make the list longer.';",
...
const eligibleContentLanes = contentLanes.filter((lane) => channelAllowedLaneIds.has(lane.id)).filter((lane) => {`,
  ...
  code = code
    ...
    .replace(
      "reason: 'complete natural cause-and-effect explanation retaining essential conditions, numbers, and technical terms', card_reason: '18-42 Korean characters: one self-contained mobile-card sentence with the single most important cause and result; no 왜:, 이유:, 핵심:, or TIP: prefix', caution: 'optional practical action only when it adds distinct value; otherwise empty; never repeat the reason'",
      "reason: 'complete 해요체 cause-and-effect explanation with an established body mechanism or clinically relevant signal and a practical decision', card_name: 'at most 14 Korean characters: common everyday Korean image label, and the same kind of thing the hook_title promises — a mistake if the title promises mistakes, an action if the title promises a method', card_reason: '24 to 38 Korean characters counted with spaces: one complete 해요체 sentence naming the condition, what happens, and what the reader should do or conclude; explicit and slightly long beats short and unclear; keep every particle and the ending; no 왜:, 이유:, 핵심:, or TIP: prefix', caution: 'optional 해요체 boundary or action only when it adds distinct value; otherwise empty; never repeat the reason'",
    )
    .replace(
      'Each ranked item must add a genuinely different point. Explain the practical cause and effect in one easy complete Korean sentence. Write image-ready Korean: concise enough to read on a phone, but preserve any condition, number, medical term, or factual detail needed for accuracy. Do not add mechanical labels such as 왜:, 이유:, 핵심:, or TIP: to every item; the sentence itself should read naturally. Keep only as many items as the topic naturally supports, between 3 and 5.',
...
const cardRows = sortedItems
  ...
  return {
    channel: '건강장수비결',
    subscription: '건강하게 나이 드는 습관을 매일 하나씩 전해 드려요. 구독해 두시면 놓치지 않고 받아보실 수 있어요.',
...
const posterReadabilityInstruction = [
  'CARD_STYLE_LOCK_V1: one illustration language for the whole card. The panel, the rank badges, the dividing rules and every row object are drawn in the same flat illustrated infographic style with soft rounded shapes; rank badges are clean solid shapes with the rank numeral, never torn paper, never tape strips, never scrapbook collage. A photographed scene may exist only OUTSIDE the panel as the room backdrop. FORBIDDEN inside the panel: photographic or 3D-rendered objects, anatomy-textbook organ diagrams, emoji, and any second drawing style.',
...
const visibleText = [`,
  ...
  // (유튜브 설명이 쓴다) 카드 표시 텍스트에서만 뺀다.
  code = code.replace(/^  'SUBTITLE: ' \+ imageSubtitle,\n/m, '');
  if (code.includes("'SUBTITLE: '")) {
    throw new Error(`${target.id}: card visible text still carries a subtitle line`);
  }
  code = code.replace(
