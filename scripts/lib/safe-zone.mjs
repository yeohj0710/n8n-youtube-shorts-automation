...
export function shortsSafeZonePromptLines(width = 1080, height = 1920) {
  ...
  return [
    ...
    // 2026-08-05 실측 교정: "at or below y 230"으로 썼더니 모델이 패널을 y 455에서
    // 시작하고 그 위 225px을 사진으로 채웠다. 상자 위쪽을 버린 만큼 아래가 밀려서
    // 마지막 행이 하단 밴드로 내려갔다. 위아래 좌표를 둘 다 못 박는다.
    `CONTENT_PANEL_V1: draw one rounded panel and put every supplied Korean letter inside it. ITS TOP EDGE SITS AT y ${box.top} AND ITS BOTTOM EDGE SITS AT y ${box.bottom}. Both numbers are fixed: do not float the panel lower to make room for a bigger photograph above it, and do not stretch it past y ${box.bottom}. The photographed background shows only in the ${box.topInset} px above the panel and the ${box.bottomInset} px below it. Treat the panel as a physical container: rows stack inside it and the last row ends above its bottom edge. The panel is therefore exactly ${box.bottom - box.top} px tall — measure it before you draw anything else. Growing it downward is the failure that keeps repeating: a measured frame put the top edge near y ${box.top} but ran the bottom to y 1655, which dropped the footer line into the strip the app covers. If the rows do not fit, the panel neither moves nor grows; make the rows shorter instead.`,
    ...
    `PANEL_FILL_V1: the rows fill the panel from just under the title to just above the footer line. Do not leave a wide blank band between the last row and the footer, and do not inset the rows far from the panel's own edges — a measured card left both and read as text floating in an empty box. Space the rows evenly so the last one lands at the bottom of the stack, and let the row objects occupy the outer half of each row that the short item names leave open.`,
    'TITLE_ZONE_CAP_V1: the title zone takes at most one third of the footprint height, and about one quarter is the target. Set the title in at most 2 lines. There is NO subtitle on this card — do not invent a second line under the title. The space it used to take belongs to the rows. A published frame spent nearly half the card on a five-line title and starved the ranked rows; that is a failure. Leftover vertical space always goes to the ranked rows, never to enlarging the title further.',
    ...
    `GLYPH_INTEGRITY_V1: Korean type below about 3 percent of frame height renders with broken strokes, and at this card's viewing size anything under ${Math.round(height * 0.026)} px is unreadable in a feed. Set card_reason at ${Math.round(height * 0.026)} px or larger and item names at ${Math.round(height * 0.036)} px or larger. THESE ARE FLOORS, NOT TARGETS — go bigger whenever the box allows. A card_reason may wrap onto a second line at that size; it never takes a third. If the copy will not fit at those sizes, cut in this order: ornament and flourishes first, then row padding, then the size of the row objects. Never cut the row objects away entirely — a card with no pictures at all reads as bare text. Shrinking the Korean type is the last resort and never below the floor; a frame with unreadable rows has failed even when every rule above is satisfied.`,
    // 2026-08-06 실측: 이 규칙이 그림을 "작은 표시일 뿐"이라 부르고 GLYPH_INTEGRITY_V1이
    // "모자라면 일러스트부터 버려라"고 해서, 모델이 아예 한 장도 안 그렸다. 카드가 글자만
    // 남아 허전해졌다(사용자 지적). 이제는 그리라고 말한다. 폭 상한은 유지한다 — 그림이
    // 열을 차지하면 문장이 한 줄 늘고 그만큼 글자가 작아지기 때문이다.
    `ROW_ICON_V1: every ranked row carries one small illustrated object that shows what the row is about, placed on the side opposite the rank badge and centred on the row. Draw it between ${Math.round(width * 0.11)} px and ${Math.round(width * 0.17)} px wide — big enough to read at a glance, never a column. Item names on this card are short, so without that object the outer half of every row sits empty and the whole card reads as bare text. All the objects share ONE drawing style — the same flat illustrated language as the badges and the panel itself. Never photographic, never 3D-rendered, never an anatomical diagram: a measured card mixed a semi-photographic leg, a cartoon foot and two anatomy-textbook organs in one list and read as four different cards collaged together. They sit beside the Korean line, never on top of it, and the sentence still gets at least ${Math.round(width * 0.55)} px of line width for each of its at most two lines.`,
